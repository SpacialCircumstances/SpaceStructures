...
var Gradient = $hxClasses["Gradient"] = function(v,c) {
	this.color = c;
	this.value = v;
};
...
var GradientMap = $hxClasses["GradientMap"] = function() {
	this.map = new List();
	this.fillStd();
};
...
var LinearImageBuilder = $hxClasses["LinearImageBuilder"] = function() {
};
...
Main.__name__ = true;
Main.main = function() {
	kha_System.init({ title : "Space Structures", width : 1200, height : 900},function() {
		new Project();
	});
};
Math.__name__ = true;
var Project = $hxClasses["Project"] = function() {
...
Project.prototype = {
	...
	,createImage: function() {
		...
		this.map.add(new Gradient(1,-16777216));
		this.map.add(new Gradient(0,-16711681));
		this.builder = new LinearImageBuilder();
		this.bgimg = this.builder.generateImage(this.map,1200,900);
		this.stars = kha_Image.createRenderTarget(1200,900);
		var g2 = this.stars.get_g2();
		g2.begin(null,0);
		g2.set_color(-1);
		var _g = 0;
		while(_g < 50) {
			++_g;
			var x = kha_math_Random.getFloatIn(0,1200);
			var y = kha_math_Random.getFloatIn(0,400);
			g2.fillRect(x,y,2,2);
		}
		g2.end();
	}
...
var Util = $hxClasses["Util"] = function() { };
Util.__name__ = true;
...
kha_Shaders.init = function() {
	var data = Reflect.field(kha_Shaders,"painter_colored_vertData");
	var bytes = haxe_Unserializer.run(data);
	kha_Shaders.painter_colored_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes),"painter_colored_vert");
	var data1 = Reflect.field(kha_Shaders,"painter_colored_fragData");
	var bytes1 = haxe_Unserializer.run(data1);
	kha_Shaders.painter_colored_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes1),"painter_colored_frag");
...
kha_SystemImpl.convertCode = function(code) {
	switch(code) {
	case 256:
		return kha_Key.ESC;
	case 260:case 261:
		return kha_Key.ENTER;
	case 274:
		return kha_Key.LEFT;
	case 275:
		return kha_Key.UP;
	case 276:
		return kha_Key.RIGHT;
	case 277:
		return kha_Key.DOWN;
	case 288:
		return kha_Key.SHIFT;
	default:
		return null;
	}
};
kha_SystemImpl.keyboardDownCallback = function(code,charCode) {
	var key = kha_SystemImpl.convertCode(code);
	if(key != null) {
		kha_SystemImpl.keyboard.sendDownEvent(key," ");
	} else {
		kha_SystemImpl.keyboard.sendDownEvent(kha_Key.CHAR,String.fromCharCode(charCode));
	}
};
kha_SystemImpl.keyboardUpCallback = function(code,charCode) {
	var key = kha_SystemImpl.convertCode(code);
	if(key != null) {
		kha_SystemImpl.keyboard.sendUpEvent(key," ");
	} else {
		kha_SystemImpl.keyboard.sendUpEvent(kha_Key.CHAR,String.fromCharCode(charCode));
	}
};
...
kha_audio2_Audio._init = function() {
	kha_audio2_Audio.buffer = new kha_audio2_Buffer(8192,2,44100);
};
...
kha_audio2_Audio1._mix = function(samples,buffer) {
	...
	while(_g4 < _g11.length) {
		...
		while(_g31 < _g21) {
			var i3 = _g31++;
			var _g41 = i3;
			kha_audio2_Audio1.sampleCache2[_g41] += kha_audio2_Audio1.sampleCache1[i3] * channel.get_volume();
		}
	}
	var _g5 = 0;
	...
	while(_g5 < _g12.length) {
		...
		while(_g32 < _g22) {
			var i4 = _g32++;
			var _g42 = i4;
			kha_audio2_Audio1.sampleCache2[_g42] += kha_audio2_Audio1.sampleCache1[i4] * channel1.get_volume();
		}
	}
	var _g13 = 0;
	var _g6 = samples;
			...
			if(this.lookupType == 1) {
				...
				if(sequenceP) {
					...
					while(_g11 < _g3) {
						...
						var _g21 = pInter;
						residueBuffers[cInter][_g21] = residueBuffers[cInter][_g21] + val2;
						if(++cInter == ch) {
							cInter = 0;
							++pInter;
						}
				...
				if(sequenceP) {
					...
					while(_g12 < _g4) {
						...
						var _g22 = pInter;
						residueBuffers[cInter][_g22] = residueBuffers[cInter][_g22] + val3;
						if(++cInter == ch) {
							cInter = 0;
							++pInter;
						}
					...
					while(_g13 < _g5) {
						...
						var _g23 = pInter;
						residueBuffers[cInter][_g23] = residueBuffers[cInter][_g23] + val4;
						if(++cInter == ch) {
							cInter = 0;
							++pInter;
						}
...
kha_audio2_ogg_vorbis_data_Header.read = function(decodeState) {
	...
	while(_g11 < _g4) {
		var i2 = _g11++;
		header1.codebooks[i2] = kha_audio2_ogg_vorbis_data_Codebook.read(decodeState);
	}
	var x6;
	if(decodeState.validBits < 0) {
		x6 = 0;
...
var kha_graphics4_FragmentShader = $hxClasses["kha.graphics4.FragmentShader"] = function(source,name) {
	this.shader = Krom.createFragmentShader(source.bytes.b.bufferValue,name);
};
...
var kha_graphics4_GeometryShader = $hxClasses["kha.graphics4.GeometryShader"] = function(source,name) {
	this.shader = Krom.createGeometryShader(source.bytes.b.bufferValue,name);
};
...
kha_graphics4_TextShaderPainter.prototype = {
	...
	,drawString: function(text,opacity,color,x,y,transformation,fontGlyphs) {
		...
		while(_g1 < _g) {
			var i = _g1++;
			var q = font.getBakedQuad(kha_graphics4_TextShaderPainter.findIndex(this.charCodeAt(i),fontGlyphs),xpos,y);
			if(q != null) {
				...
				var x8 = (transformation._00 * x7 + transformation._10 * y7 + transformation._20) / w3;
				var y8 = (transformation._01 * x7 + transformation._11 * y7 + transformation._21) / w3;
...
kha_graphics4_IndexBuffer.prototype = {
	...
	,unlock: function() {
		Krom.setIndices(this.buffer,this.indices);
	}
	,set: function() {
...
var kha_graphics4_PipelineState = $hxClasses["kha.graphics4.PipelineState"] = function() {
	...
	this.program = Krom.createProgram();
};
...
kha_graphics4_PipelineState.prototype = $extend(kha_graphics4_PipelineStateBase.prototype,{
	'delete': function() {
		Krom.deleteProgram(this.program);
		this.program = null;
	}
	,compile: function() {
		...
		Krom.compileProgram(this.program,structure0,structure1,structure2,structure3,this.inputLayout.length,this.vertexShader.shader,this.fragmentShader.shader,gs,tcs,tes);
	}
	,set: function() {
		Krom.setProgram(this.program);
	}
	,getConstantLocation: function(name) {
		return Krom.getConstantLocation(this.program,name);
	}
	,getTextureUnit: function(name) {
		return Krom.getTextureUnit(this.program,name);
	}
...
var kha_graphics4_TessellationControlShader = $hxClasses["kha.graphics4.TessellationControlShader"] = function(source,name) {
	this.shader = Krom.createTessellationControlShader(source.bytes.b.bufferValue,name);
};
...
var kha_graphics4_TessellationEvaluationShader = $hxClasses["kha.graphics4.TessellationEvaluationShader"] = function(source,name) {
	this.shader = Krom.createTessellationEvaluationShader(source.bytes.b.bufferValue,name);
};
...
kha_graphics4_VertexBuffer.prototype = {
	...
	,unlock: function() {
		Krom.setVertices(this.buffer,this.vertices);
	}
	,stride: function() {
...
var kha_graphics4_VertexShader = $hxClasses["kha.graphics4.VertexShader"] = function(source,name) {
	this.shader = Krom.createVertexShader(source.bytes.b.bufferValue,name);
};
...
var kha_input_Gamepad = $hxClasses["kha.input.Gamepad"] = $hx_exports["kha"]["input"]["Gamepad"] = function(id) {
	if(id == null) {
		id = 0;
	}
	this.axisListeners = [];
...
kha_krom_Graphics.getBlendFunc = function(op) {
	return 0;
};
kha_krom_Graphics.prototype = {
	...
	,setDepthMode: function(write,mode) {
		Krom.setDepthMode(write,mode[1]);
	}
	,getBlendingMode: function(op) {
		switch(op[1]) {
		case 0:case 1:
			return 0;
		case 2:
	...
	,setBlendingMode: function(source,destination) {
		Krom.setBlendingMode(this.getBlendingMode(source),this.getBlendingMode(destination));
	}
	,setVertexBuffer: function(vertexBuffer) {
		vertexBuffer.set(0);
	}
	,setVertexBuffers: function(vertexBuffers) {
		Krom.setVertexBuffers(vertexBuffers.length > 0 ? vertexBuffers[0].buffer : null,vertexBuffers.length > 1 ? vertexBuffers[1].buffer : null,vertexBuffers.length > 2 ? vertexBuffers[2].buffer : null,vertexBuffers.length > 3 ? vertexBuffers[3].buffer : null,vertexBuffers.length);
	}
	...
	,setPipeline: function(pipeline) {
		...
		this.setColorMask(pipeline.colorWriteMaskRed,pipeline.colorWriteMaskGreen,pipeline.colorWriteMaskBlue,pipeline.colorWriteMaskAlpha);
		pipeline.set();
	}
	...
	,setStencilParameters: function(compareMode,bothPass,depthFail,stencilFail,referenceValue,readMask,writeMask) {
		...
		Krom.setStencilParameters(compareMode[1],bothPass[1],depthFail[1],stencilFail[1],referenceValue,readMask,writeMask);
	}
	,scissor: function(x,y,width,height) {
...
kha_math_FastMatrix4.lookAt = function(eye,at,up) {
	...
	return new kha_math_FastMatrix4(xaxis.x,xaxis.y,xaxis.z,-(xaxis.x * eye.x + xaxis.y * eye.y + xaxis.z * eye.z),_x1,_y1,_z1,-(_x1 * eye.x + _y1 * eye.y + _z1 * eye.z),-zaxis.x,-zaxis.y,-zaxis.z,zaxis.x * eye.x + zaxis.y * eye.y + zaxis.z * eye.z,0,0,0,1);
};
...
kha_math_Matrix4.lookAt = function(eye,at,up) {
	...
	var _y1 = xaxis.z * zaxis.x - xaxis.x * zaxis.z;
	var _z1 = xaxis.x * zaxis.y - xaxis.y * zaxis.x;
	return new kha_math_Matrix4(xaxis.x,xaxis.y,xaxis.z,-(xaxis.x * eye.x + xaxis.y * eye.y + xaxis.z * eye.z),_x1,_y1,_z1,-(_x1 * eye.x + _y1 * eye.y + _z1 * eye.z),-zaxis.x,-zaxis.y,-zaxis.z,zaxis.x * eye.x + zaxis.y * eye.y + zaxis.z * eye.z,0,0,0,1);
};
...
Project.screenWidth = 1200;
Project.screenHeight = 900;
...
kha_Shaders.painter_colored_vertData = "s1303:AnZlcnRleENvbG9yAAB2ZXJ0ZXhQb3NpdGlvbgABASRHbG9iYWxzAAABcHJvamVjdGlvbk1hdHJpeAAAQERYQkPKNKCZBL%hzmJdxhiWjM6pAQAAAJQDAAAFAAAANAAAAAwBAABYAQAAsAEAABgDAABSREVG0AAAAAEAAABIAAAAAQAAABwAAAAABP7::IQEAAAAAAAXwAAA3IQEAABAAAAZQAAA:IgEAAAAAAAZwAABPIgEAABAAAAAQAAAGgAAAICAAAANgAABfIgEAAAAAAARh4QAAAAAAA2AAAFcgAQAAAAAABGEhAAAQAAADYAAAWCABAAAAAAAAFAAAAAAIA:";
kha_Shaders.painter_colored_fragData = "s580:AAAARFhCQxK2flkyOc:%i:tdk83zcfgBAAAAsAEAAAUAAAA0AAAAjAAAAMAAAAD0AAAANAEAAFJERUZQAAAAAAAAAAAAAAAAAAAAHAAAAAAE::";
